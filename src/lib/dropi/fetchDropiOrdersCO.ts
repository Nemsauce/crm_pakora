...
const DROPI_ORDERS_URL = "https://api.dropi.co/api/orders/myorders";
const PAGE_SIZE = 50;
const MAX_PAGES = 20;
...
async function fetchOrdersPage(token: string, page: number) {
  ...
  return payload.objects as DropiOrderCO[];
}

export async function fetchDropiOrdersCO(token: string) {
  const orders: DropiOrderCO[] = [];
