...
const DROPI_API_BASE_URL = "https://api.dropi.mx/api";
...
export type DropiAuthMXWithTokenResult =
...
function failure(
  step: string,
  errorMessage: string,
): Extract<DropiAuthMXWithTokenResult, { success: false }> {
  console.error(`${step}: failed - ${errorMessage}`);

  return { success: false, errorMessage };
}

export async function dropiAuthMXWithToken(): Promise<DropiAuthMXWithTokenResult> {
  const email = process.env.DROPI_EMAIL_MX;
  const password = process.env.DROPI_PASSWORD_MX;
  const totpSecret = process.env.DROPI_TOTP_SECRET_MX;

  if (!email || !password || !totpSecret) {
    return failure(
      "dropi configuration",
      "Missing DROPI_EMAIL_MX, DROPI_PASSWORD_MX, or DROPI_TOTP_SECRET_MX",
    );
  }
...
export async function dropiAuthMX(): Promise<DropiAuthResult> {
  const result = await dropiAuthMXWithToken();
