...
function matchDropiOrders(
  ...
  function matchRemainingById(
    ...
    getDropiKey: (order: DropiOrderCO) => string | null,
  ...
  const dropiGroups = new Map<
    string,
    Array<{ index: number; order: DropiOrderCO }>
  >();
...
function getRiskLevel(dropiOrder: DropiOrderCO) {
...
async function loadActiveCOOrders() {
...
export async function syncDropiOrdersCO(
  dropiOrders: DropiOrderCO[],
): Promise<SyncDropiOrdersCOResult> {
  const supabaseOrders = await loadActiveCOOrders();
