...
### [CRM v4 — Gate G0] Aislamiento y red de regresión read-only — COMPLETADO; staging mutable pendiente
- Se fijó la línea base `3ffd66aad1cb0a11f4228b439f07e512b877b255`, se creó el tag anotado `pre-crm-v4-20260728` y la rama aislada `redesign/crm-v4`. Ningún cambio parcial se aplicó a `main`.
- Se creó `REDESIGN_CONTRACTS.md` como matriz rastreada de rutas, parámetros de URL, deep links, acciones mutables, fuentes de datos, fórmulas, estados visuales y dependencias que el rediseño debe preservar.
- Se instaló Playwright y Axe como dependencias de desarrollo y se configuró la matriz Chromium para escritorio, tablet y móvil en claro/oscuro, además de reduced motion. Las seis capturas iniciales del login se revisaron visualmente y quedaron versionadas como línea base; el indicador de diagnóstico de Next.js se excluye de forma explícita porque no pertenece a la aplicación.
- Se añadió un guard fail-closed para cualquier prueba mutable: exige habilitación literal, entorno Vercel no productivo, refs diferentes y exactas de Supabase staging/producción, service role y un marker exclusivo de staging verificado mediante consulta read-only. Con el entorno local actual, el guard rechaza correctamente la ejecución y no realiza escrituras.
- Se añadió CI para instalación reproducible con pnpm 11.9.0, lint, TypeScript, build, Playwright y artefactos de fallo. La suite read-only local aprobó `35/35`; `tsc --noEmit`, `git diff --check` y el build de producción aprobaron. ESLint aprobó con los dos warnings preexistentes de `scripts/n8n/patch-dropi-migracion-historical.mjs`.
- La inspección confirmó que el repo no contiene una migración completa y reproducible del schema live de Supabase: solo hay dos SQL aditivos y el vínculo local apunta al proyecto existente. Por seguridad no se inventaron migraciones, no se ejecutó reset/seed y no se usó producción para QA.
- Se inspeccionó en modo read-only el archivo Figma `CRM Pakora v4`: está vacío, admite Space Grotesk, Manrope y JetBrains Mono, y tiene disponibles Material 3 y Simple Design System como bibliotecas de referencia. La creación de foundations queda pausada en el checkpoint de aprobación exigido por el workflow de Figma.
- Pendiente para cerrar G0 integral: proyecto Supabase staging separado con schema auditado, usuario Auth de Playwright, marker y fixtures deterministas; variables de Vercel Preview apuntando exclusivamente a staging. Hasta entonces solo están habilitadas pruebas públicas/read-only.

### [CRM v4 — Gate G0] Caracterización pública de Auth — COMPLETADO
- La línea base read-only cubre ahora login normal, error recibido por query param, ausencia de registro público, invitación inválida en `/set-password`, retorno al login, persistencia de tema, overflow responsive, orden de foco por teclado y configuración real de `prefers-reduced-motion`.
- Se añadieron y revisaron 12 snapshots nuevos para login con error e invitación inválida en claro/oscuro y 375/768/1440 px. Junto con el login normal son 18 snapshots estables; la suite acumulativa aprobó `72` pruebas con `7` skips intencionales del caso exclusivo de reduced motion.
- Axe encontró una deuda preexistente y precisa en el error del login light: texto rojo `#DC2626` sobre fondo `#FEE2E2`, contraste `3.95:1` frente al mínimo `4.5:1`. La prueba registra exactamente esa única violación y exige que dark siga limpio; G3 deberá eliminar esta expectativa al corregir los tokens, no conservarla ni ampliarla.
- Los flujos Auth mutables —login real, invitación válida, cambio de contraseña y logout— continúan bloqueados hasta disponer de Supabase staging. No se consultaron secretos ni se realizaron mutaciones productivas.

### [CRM v4 — Gate G0] Harness staging, readiness y fixtures lógicos — CORTE LOCAL COMPLETADO; G0 externo pendiente
- Se endureció la suite pública para que solo arranque en localhost con `NEXT_PUBLIC_SUPABASE_URL`, anon key y service role vacíos. El fixture bloquea cualquier request de navegador fuera del origen local e intercepta/cierra WebSockets no permitidos antes de conectarlos a un servidor; el fixture staging aplica la misma protección limitada al Preview y Supabase atestiguados. Los proyectos Auth/mutables ni siquiera se cargan en modo público. CI sanea también el build y verifica que un entorno staging vacío falle cerrado.
- La ejecución mutable ahora tiene dos capas no omitibles: `globalSetup` valida antes de cualquier test y la cadena Playwright conserva `staging-guard → auth → mutable`. Se reprodujo el intento con `--no-deps` y fue rechazado antes de abrir navegador. La allowlist positiva de refs staging queda deliberadamente vacía hasta tener el proyecto real auditado; ninguna combinación de variables de entorno puede habilitar mutaciones por sí sola.
- Se añadió `/api/e2e/attestation`, activo únicamente en Preview y protegido con un secreto staging. Omite de forma exacta el redirect de sesión del middleware porque el runner no porta cookie Supabase; el endpoint conserva su propia doble validación de secreto y marker. El propio deployment debe atestiguar el mismo origen/ref Supabase esperado por el runner; redirects, producción, ref distinta o marker ausente bloquean la suite. Sus ramas `404/401/503/409/200` tienen pruebas directas y el servidor local prueba por HTTP el `404` no-Preview sin seguir redirects; el recorrido live queda pendiente del Preview aislado. Auth exige además el destino `/pedidos`, la navegación Principal y el heading real; sus trazas, videos y screenshots están deshabilitados para no capturar credenciales.
- Se creó una auditoría AST read-only contra `database.types.ts`. La deuda conocida dentro de llamadas literales `.from()`/`.rpc()` quedó versionada: 8 tablas, 5 RPC y 8 columnas live ausentes del tipo generado; cualquier gap literal nuevo no allowlisted falla. Wrappers y nombres dinámicos no quedan cubiertos y por eso siguen siendo obligatorios el dump auditado y la regeneración real de tipos. El scanner excluye expresamente los archivos no rastreados protegidos y no se conectó a Supabase.
- Se creó un contrato lógico de fixtures, determinista, profundamente inmutable, anclado en `America/Bogota`, con claves `FX-*` y sin PK de base. Cubre CO/MX, estados/riesgos, las 13 categorías de estado, tareas, `99+` notificaciones, WhatsApp, abandonados, wallet, productos y costeos. Los cálculos internos quedaron nombrados y documentados como oráculos de escenario; no se presentan como pruebas de fórmulas privadas ni RPC. La paridad productiva real cubre hoy historial Dropi, resultados permitidos de tareas y formato WhatsApp, incluido `#1007: 118 = 39 + 78 + 1` contra `getCustomerHistoryStats`. Su adaptador mutable permanece bloqueado hasta contar con schema auditado, tipos regenerados, usuarios Auth, RLS y marker reales.
- Verificación de este corte: build de producción aprobado; Playwright público `102` aprobadas y `7` skips intencionales; proyecto funcional dentro de esa matriz `41` aprobadas y `1` skip intencional; `tsc --noEmit`, scripts Node y `git diff --check` aprobaron. ESLint completo terminó con cero errores y conserva únicamente los dos warnings preexistentes del script histórico n8n. El gate G0 integral sigue pendiente de provisioning externo, regeneración segura del schema/tipos, ejecución mutable real en Preview staging y autenticación de GitHub para publicar la rama/draft PR.
