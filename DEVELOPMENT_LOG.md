...
### [Rediseño CRM v5] Fase 10 — motion y profundidad de torre de control — COMPLETADO
- Se añadió una gramática compartida de movimiento y profundidad para listas, drawers, hover táctil, CTA primario, confirmación de tareas, loading y fondo ambiental. Las rutas continuas usan `transform`/`opacity`; el shimmer también barre mediante un pseudo-elemento transformado para evitar repaints de `background-position`.
- Finanzas, Hoy y los resultados de Costeos/Proyección incorporan count-up real de `600–900ms`, con ancho final reservado, formato monetario intacto y valor final inmediato cuando `prefers-reduced-motion` está activo. La tendencia diaria real de Finanzas conserva sus datos y suma reveal de línea/área con gradiente.
